...
interface RoastLevelCardCardLockProps {
  className?: string;
  refetch: () => Promise<void>;
}

const RoastLevelCardCardLock: FC<RoastLevelCardCardLockProps> = ({
  className,
  refetch,
}) => {
  ...
  return (
    <Card className={cn(className, "relative min-h-[543px]")}>
      <NoPermissionDashboard
        title={t("roastLevel")}
        subTitle={t("roastLevelLockDescription")}
        beans={data?.price}
      ...
      />
      <Image
        src="/images/blur/roastLevelBlur.png"
        fill
        alt="Roast Level Blur"
...
export default RoastLevelCardCardLock;
